...
class WardController extends Controller
{
    ...
    public function wardWiseLocations(Request $request, Ward $ward)
    {
        $locations = $ward->locations()->get();

        $locationHtml = '<span>
            <option value="">Select Location</option>';
        foreach ($locations as $location) :
            $locationHtml .= '<option value="' . $location->id . '" >' . $location->location . '</option>';
        endforeach;
        $locationHtml .= '</span>';

        return ['result' => 1, 'locationHtml' => $locationHtml];
    }
}
